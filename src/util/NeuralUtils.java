...
public class NeuralUtils {
	 ...
	 */
	public static SimpleMatrix elementTanhGrad(SimpleMatrix input) {
		SimpleMatrix ones = new SimpleMatrix(input.numRows(), input.numCols());
		ones.set(1.0);
		SimpleMatrix tanh = elementwiseApplyTanh(input);
		SimpleMatrix tanhSquared = tanh.elementMult(tanh);
		return ones.minus(tanhSquared);
	}
