...
public class StereotypePValue {
	...
	private void run() {
		...
		for(String id : students.keySet()) {
			...
			addScore(allValues, s.getPreCs());
			addScore(allValues, s.getPostCs());
		}
		
		double countLarger = 0;
		for(int i = 0; i < ITERATIONS; i++) {
			List<Double> sampledPre = sample(allValues, 143);
			List<Double> sampledPost = sample(allValues, 144);
